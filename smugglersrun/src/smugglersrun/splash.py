...
class Splash(BaseScene):
    ...
    def __init__(self, *args, next_scene=None, package, **kwargs):
        super().__init__()
        c = 255, 255, 255

        font = Font(f"{package}/resources/Comfortaa_Bold.ttf", size=128)
        a = RectangleSprite(
            image=Text("A",
                       font=font,
                       color=c),
        ...
        icon = Sprite(
            image=Image(f"{package}/resources/piper.png"),
            size=5
        )
        icon.bottom = a.top + 1
        name.top = a.bottom
